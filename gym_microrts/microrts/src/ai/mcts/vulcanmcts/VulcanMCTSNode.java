 ...
 * @author santi
 */
public class VulcanMCTSNode extends MCTSNode {
    ...
    HashMap<BigInteger,VulcanMCTSNode> childrenMap = new LinkedHashMap<BigInteger,VulcanMCTSNode>();    // associates action codes with children
    ...
    public VulcanMCTSNode(int maxplayer, int minplayer, GameState a_gs, VulcanMCTSNode a_parent, double a_evaluation_bound, int a_creation_ID, boolean fensa) throws Exception {
        ...
        if (gs.winner() != -1 || gs.gameover()) {
            ...
            type = -1;
            System.err.println("VulcanMCTSNode: This should not have happened...");
        }
    }

    
    // Naive Sampling:
    public VulcanMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
    ...
    public VulcanMCTSNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat()>=epsilon_g) {
            ...
            VulcanMCTSNode best = (VulcanMCTSNode)children.get(r.nextInt(children.size()));
            return best;
        }
    }
    ...
    public VulcanMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {   
        PlayerAction pa2;
        ...
        VulcanMCTSNode pate = childrenMap.get(actionCode);
        if (pate==null) {
            actions.add(pa2);            
            GameState gs2 = gs.cloneIssue(pa2);
            VulcanMCTSNode node = new VulcanMCTSNode(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, a_creation_ID, forceExplorationOfNonSampledActions);
    ...
    public void propagateEvaluation(double evaluation, VulcanMCTSNode child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (parent != null) {
            ((VulcanMCTSNode)parent).propagateEvaluation(evaluation, this);
        }
    }
